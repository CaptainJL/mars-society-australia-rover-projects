...
def log():
	...
	file.write('time,command,temp_ambient,ax,ay,az,compass,motor_current_m1,motor_current_m2,mlx_ambient,mlx_object\n')
	ax,ay,az = motion.accelerometer()
	time.sleep(0.1)
	while True:
    		ax,ay,az = motion.accelerometer()
    		file.write('%s,%s,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f,%0.2f\n' % (time.strftime('%M:%S'),cmd_val[0],weather.temperature(),9.81*ax,9.81*ay,9.81*az,motion.heading(),analog.read(0),analog.read(1),mlxsensor.get_amb_temp(),mlxsensor.get_obj_temp()))
    		time.sleep(0.1)
